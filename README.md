...
By Kirchhoff’s Current Law (KCL), For the $j^{th}$ column, and $m^{th}$ (near the ground), word line, the current is the sum of currents from all (previous) wordlines $I_{ij},\ i=1..m,\ $ that flow in this $mj^{th}$ biltline node. Hence, in ideal case with zero wire resistances,
